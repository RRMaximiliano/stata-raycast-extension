...
export default function Command() {
    ...
    async function findDatasets() {
      ...
      try {
        ...
        const uniqueDatasets = allDatasets.filter(
          (dataset, index, self) => index === self.findIndex((d) => d.path === dataset.path),
        );
  ...
  async function openInStata(filePath: string) {
    try {
      exec(
        `osascript -e 'tell application "StataMP" to activate' -e 'tell application "System Events" to keystroke "u" using {command down}' -e 'delay 0.5' -e 'tell application "System Events" to keystroke "${filePath.replace(/"/g, '\\"')}"' -e 'tell application "System Events" to keystroke return'`,
      );
        ...
        >
          {files.map((dataset) => (
            <List.Item
              ...
              actions={
                <ActionPanel>
                  <Action title="Open in Stata" icon={Icon.Play} onAction={() => openInStata(dataset.path)} />
                  <Action title="Show in Finder" icon={Icon.Finder} onAction={() => showInFinder(dataset.path)} />
                  <Action.CopyToClipboard title="Copy File Path" content={dataset.path} icon={Icon.Clipboard} />
                  ...
                  <Action
                    ...
                    onAction={() => {
                      exec(
                        `osascript -e 'tell application "StataMP" to activate' -e 'tell application "System Events" to keystroke "use \\"${dataset.path.replace(/"/g, '\\"')}\\", clear" & return' -e 'delay 0.5' -e 'tell application "System Events" to keystroke "describe" & return'`,
                      );
                    }}
                  />
                </ActionPanel>
